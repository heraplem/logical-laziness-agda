...
queue~\cite{purely-functional}. We omit definitions of
these translated functions for space, but interested readers can view an insertion sort case study
 in \cref{apx:case-study-insertion-sort}, and more in our
...
In Curry, there are two ways of implementing the logic demand functions: 
...
Although the \transDname{} is simpler than the original demand translation,
the question remains whether it is reasonably efficient, as it depends on the
\transCname{} and free variables. To evaluate\lai{Maybe choose another word, since 'evaluate' is a technical term} this, we ran \cdc{insertionSortD}
on lists of the form \cdc{[n, n-1 .. 1]} with sizes ranging from \(1\) to
\(100\), demanding the full list each time. We show the result of our
...
timing functionality of KiCS2 (version 3.5.0), running each evaluation once.
...
demand translation.
%, which, in the worst case, must call the pure \cdc{insertionSort} function \(n\) times.
However, the constants are much worse: \cdc{insertionSortD} takes \(6.44\)
seconds on a \(100\)-element list, while the 
original demand translation version in Curry is fast enough that its runtime cannot be
reliably measured.
...
general-purpose logic language. Nevertheless, the experiment shows that the
\transDname{} achieves acceptable performance~(worst performance on testing a
fully out-of-order list with 100 elements is 6.44 seconds) and reveals
potential optimization opportunities for Curry compilers.
