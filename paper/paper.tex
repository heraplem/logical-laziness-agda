...
\begin{abstract}
  Multiple cost models for lazy programs exist, but translating between them typically proves challenging. Two of these models, clairvoyance semantics and demand semantics, are proven to be time-cost equivalent, but a simple, provably correct translation between the two had yet to be discovered. We present a means of representing lazy programs in functional logic
  languages which allows us to achieve that simple translation. Terms in this representation simultaneously implement both
  semantics for lazy programs, with only a small modification needed to translate from one to the other. Our contribution enables previously unattained flexibility between the two models, and 
  represents an advancement in cost analysis of lazy programs. All of our results
  have been formalized in the proof assistant, Agda.  \keywords{Functional Logic
    Programming, Lazy Evaluation, Agda, Cost Analysis, Semantics}\lai{Too many
    keywords? Too few?}
\end{abstract}
