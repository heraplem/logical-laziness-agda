...
Suppose that we wanted to analyze the cost of this program. As explained prior,
we should expect the final time cost to be \(O(n)\), despite [1..]'s naive
non-termination. To do this, we can perform the transformation outlined
...
program that [TODO: succinctly write benefit of Demand Semantics here later]. If
that is the case, we would like to be able to use the method detailed
in~\cite{demand-semantics} instead. However, this leaves us with some unresolved
...
Rather than defining insertD naively through transforming insertA, suppose that
we instead took full advantage of the nondeterminism [TODO: explain the Curry
benefits better] that Curry offers. Indeed, it turns out that the following is
equivalent to a directly translated version of insertD:

[TODO: Curry insertD goes here]

Now, all we need to do is prove that this version of insertD is also correct and
time-cost equivalent, and we will have solved our problem. How we do so is
detailed in sections 3-5. \lai{TODO: Too informal :/}
