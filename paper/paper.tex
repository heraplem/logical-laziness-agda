...
An early example is the \emph{demand semantics} of Bjerner and Holmstr{\"{o}}m.  In this semantics, an open term is represented as a function that takes, in addition to a list of variable bindings, a \emph{demand} on the value to which the term will ultimately reduce; it produces a list of demands on its free variables.  The demand represents, essentially, ``how much'' of the result will end up being used by the rest of the program.  This semantics was originally presented without types; a version with types was published in <<STORY_OF_YOUR_LAZY_FUNCTIONS_LIFE>>.
...
In <<STORY_OF_YOUR_LAZY_FUNCTIONS_LIFE>>, the fact that demand semantics is equivalent to the standard semantics was established by proving equivalence to clairvoyance semantics, which had already been proven equivalent to the standard semantics.  However, this led to some difficulties with mechanized proofs.  These proofs required writing demand functions by hand, a tedious and error-prone process.  In fact, it required writing three versions of ``the same term'': a pure version, a clairvoyance version, and a demand version.  Thus, we sought alternate means of expressing demand functions.  In particular, we hoped to be able to write a term \emph{once} and automatically obtain the clairvoyance and demand versions of that term.
