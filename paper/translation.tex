...
We now define another translation from \( \LB \) to \( \LL \) that is intended to simulate demand semantics. Given
\( \TB{\Gamma}{t}{A} \), the demand translation of \( t \) introcuces new logic variables with types corresponding to
the types in \( \Gamma \); consequently, \( \Gamma \) must become a parameter to the translation. We therefore denote the demand
translation by \( \transD{t}{\Gamma} \).
