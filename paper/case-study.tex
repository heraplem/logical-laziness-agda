...
queue~\cite{purely-functional}. For the sake of space, we omit definitions of
these translated functions, but interested readers can find a case study on
insertion sort in \cref{apx:case-study-insertion-sort} and all the code in our
...
including all the logical clairvoyance functions. In addition, even though the
...
we can always find the least result via a depth-first search.
...
In Curry, there are two ways of implementing the logic demand functions: using
guards and using generators. Even though they are semantically equivalent, there
...
takeD' xsT ysD |  xsTD <~ xsT
  ...
  where xsTD, c free
\end{lstlisting}
In constrast to the definition in \cref{fig:takeD-curry}, we simply use the
...
input demand, which performs poorly. On the other hand, our implementation in
\cref{fig:takeD-curry} uses an efficient generator \cdc{approx} defined for the
\cdc{List} datatype, which performs much more efficient.
...
Although the \transDname{} is much simpler than the original demand translation,
a question remains whether it performs reasonably efficient as it depends on the
\transCname{} and free variables. To evaluate this, we ran \cdc{insertionSortD}
on lists of the form \cdc{[n, n-1 .. 1]} with sizes ranging from \(1\) to
\(100\), demanding the full result each time. We show the result of our
...
timing functionality of KiCS2 (version 3.5.0) by running each evaluation once.
...
seconds on a \(100\)-element list, while the version we implemented using the
original demand translation in Curry is fast enough that its runtime cannot be
reliably measured.
