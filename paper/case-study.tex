...
performance evaluation in \cref{fig:insertionSortD_runtime}. All tests were
...
demand translation, which, in the worst case, must call the pure
\cdc{insertionSort} function \(n\) times. However, the constants are much worse:
\cdc{insertionSortD} takes more than \(6\) seconds on a \(100\)-element list,
while the version we implemented using the original demand translation in Curry
is fast enough that its runtime cannot be reliably measured.
...
fully out-of-order list with 100 elements is less than 7 seconds) and reveals
