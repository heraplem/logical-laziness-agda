...
We now define another translation from \( \LB \) to \( \LL \) that is intended to simulate demand semantics. Given \( \TL{\Gamma}{t}{A} \), the demand translation of \( t \) must introduce new 
logic variables with types corresponding to the types in \( \Gamma \), so \( \Gamma \) must be a parameter to the translation; we therefore denote it by \( \transD{\Gamma}{t} \).
