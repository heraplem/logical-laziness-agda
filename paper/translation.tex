...
In this section, we define two languagues, a base language $B$ that corresponds
a simple functional language~(\cref{subsec:base-lang}) and a logic language $L$
that corresponds to a functional logic language~(\cref{subsec:logic-lang}). We
then develop a \emph{clairvoyance translation} strategy that translates a
program in $B$ to a program in $L$ that models the clairvoyance
...
\subsection{The Base Language $B$}\label{subsec:base-lang}
...
Note that $L$ is not total because \( \tfail \) never reduces.
...
\begin{figure}[t]
  \[
    \begin{array}{rcl}
      ...
      \tfoldM{x_1}{x_2}{t_1}{t_2}{t_3} &= &\tfoldr{x_1}{y_2}{\texttt{\tdo{x_2}{\ttranspose{y_2}}{t_1}}}{t_2}{t_3}
    \end{array}
  \]
...
We now define another translation from \( \LB \) to \( \LL \) that is intended to simulate demand semantics. Given \( \TL{\Gamma}{t}{A} \), the demand translation of \( t \) introcuces new 
logic variables with types corresponding to the types in \( \Gamma \); consequently, \( \Gamma \) must become a parameter. We therefore denote the demand translation by \( \transD{t}{\Gamma} \).

\begin{align*}
  \transD{t}{x_1 : A_1, \ldots, x_n : A_n} = &\texttt{let \( x'_1 \) = \tfree{A_1} in} \\
                                             &\texttt{assert \tle{x'_1}{x_1} in} \\
                                             &\vdots \\
                                             &\texttt{let \( x'_n \) = \tfree{A_n} in} \\
                                             &\texttt{assert \tle{x'_n}{x_n} in} \\
                                             &\texttt{let \( u \) = \( \transC{t'} \) in} \\
                                             &\texttt{assert \teq{x}{\tfst{u}} in} \\
                                             &\texttt{((\( x'_1 \), \( \ldots \), \( x'_n \)), \( \tsnd{u} \))} \\
  \text{where \( t' = t[x'_1/x_1, \ldots, x'_n/x_n] \).}
\end{align*}

Since the demand translation is intended to simulate demand semantics, it must evaluate to ``the same sort of thing'' as demand semantics, \ie an environment of approximations. To that end, we define an operation \( \verts{-} \) that turns contexts into types:
\[
