...
⇑⌊_⌋ᵈ : ∀ {Γ α} {t : Explicit.Tm Γ α}
          {γ : 𝕃.⟦⌊ Γ ⌋⟧ᶜ} {vᴬ u c}
        → ⟦⌊ t ⌋⟧ᵉ (γ ⸴ vᴬ) ∋ (u , c)
        → Σ[ γᴬ ∈ 𝕃.⟦⌊ Γ ⌋⟧ᶜ ] ∥ γᴬ ∥ ≡ u × γᴬ ≲ᶜ γ × ℂ.⟦⌊ t ⌋⟧ᵉ γᴬ ∋ (vᴬ , c)
⇑⌊ φ ⌋ᵈ = ⇑wrap φ

ℂ⇓⌊_,_⌋ᵈ : ∀ {Γ α} {t : Explicit.Tm Γ α}
             {γ γᴬ aᴬ c}
         → γᴬ ℂ.≤ᶜ γ
         → ℂ.⟦ t ⟧ᵉ γᴬ ∋ (aᴬ , c)
         → ⟦⌊ t ⌋⟧ᵉ (ℂ.⌊ γ ⌋ᶜ ⸴ ℂ.⌊ aᴬ ⌋ᵗ) ∋ (∥ ℂ.⌊ γᴬ ⌋ᶜ ∥ , c)
ℂ⇓⌊ ψ , φ ⌋ᵈ = ⇓⌊ ℂ.⌊ ψ ⌋≤ᶜ , ℂ.⌊ φ ⌋ᵈ ⌋ᵈ

ℂ⇑⌊_⌋ᵈ : ∀ {Γ α} {t : Explicit.Tm Γ α}
           {γ aᴬ u c}
