...
In the absence of recursion, call-by-need is semantically equivalent to call-by-name.  However, the two semantics have different performance characteristics: in particular, call-by-need is \emph{cost-optimal} in the sense that a call-by-need derivation of \( t \Downarrow v \) will never be larger than a call-by-name derivation of the same, and in fact it may be smaller.  Thus, a natural way to study call-by-name evaluation \emph{in paticular} is to augment the evaluation relation with an additional parameter that tracks the cost of evaluation.

The standard semantics for call-by-name uses a heap of variable bindings that is updated as the expressions to which variables are bound are evaluated.  This is attractive because it reflects the most straightforward implementation of call-by-name evaluation on an actual computer, but it has the problem that it is non-compositional; that is, a derivation is not generally a simple composition of subterm derivations.  Therefore, some have tried to develop semantics that are equivalent but also compositional.
...
Another example of compositional semantics is the \emph{clairvoyance semantics} of Hackett and Hutton.  The key idea of this semantics is that, when binding a variable to an expression, rather than deferring computation of the bound expression until it is needed, we instead \emph{immediately} choose nondeterministically to either evaluate it or discard it.  This semantics is equivalent to the standard semantics in the sense that, if \( t \Downarrow v \), then there is \emph{some} derivation of this fact that is cost-equivalent to the derivation of the same in the standard semantics; and, furthermore, this derivation is the minimal such derivation.
